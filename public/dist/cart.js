...
window.decrease = function (id) {
  ...
  if (item) {
    item.qty--;
    if (item.qty <= 0) window.removeItem(id);else save();
  }
};
...
function tryAutoSaveCustomer() {
  ...
  if (name || email || whatsapp) {
    const existing = (() => {
      try {
        return JSON.parse(localStorage.getItem('tocha-customer') || '{}');
      } catch (_) {
        return {};
      }
    })();
...
window.openCheckoutModal = function () {
  ...
  const stored = JSON.parse(localStorage.getItem('tocha-customer') || 'null');
  if (!stored?.email) {
    fetch('/api/customer/me').then(r => r.ok ? r.json() : null).then(c => {
      if (c?.email) {
        fillCustomerForm(c);
...
window.confirmarPedido = async function () {
  ...
  if (window.track) window.track('checkout_started', {
    method: payment,
    items: cartToCheckout.length
  });
  ...
  try {
    ...
    if (window.track) window.track('payment_attempted', {
      method: payment
    });
    ...
    if (payment === 'mp_card') {
      ...
      const res = await fetch('/api/mercadopago/prepare-card-order', {
        ...
        body: JSON.stringify({
          customer,
          cart: cartToCheckout,
          funnel_session_id: window.getSessionId ? window.getSessionId() : null
        })
      });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({
          error: 'Servidor retornou erro.'
        }));
    ...
    if (payment === 'pix') {
      ...
      const data = await res.json();
      if (window.track) window.track('payment_success', {
        method: 'stripe'
      });
    ...
    if (window.track) window.track('payment_failed', {
      method: payment,
      error: e.message
    });
...
window.toggleCart = function () {
  const drawer = document.getElementById('cartDrawer');
  if (drawer && drawer.classList.contains('open')) window.closeCart();else window.openCart();
};
...
async function applyPaymentMethodSettings() {
  try {
    ...
    const visibleOptions = Array.from(document.querySelectorAll('input[name="payment-method"]')).filter(opt => opt.closest('label')?.style.display !== 'none');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (_recoveryToken) {
    fetch(`/api/recover/${encodeURIComponent(_recoveryToken)}`).then(r => r.ok ? r.json() : null).then(data => {
  ...
  if (params.get('status') === 'success') {
    ...
    if (mpPaymentId) {
      ...
      fetch(`/api/mercadopago/check-payment/${mpPaymentId}?order_id=${mpOrderId}`).then(r => r.json()).then(d => {
        if (!d.valid) {
